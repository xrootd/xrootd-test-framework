...
class XrdTestSlave(Runnable):
    ...
    def executeSh(self, cmd):
        ...
        try:
            ...
            res = (output, '', str(retcode), logs, coreinfo)
    ...
    def checkCoreDumps(self):
        '''Search for any core dumps produced by the previous process.

        Core file location: /tmp/cores/
        Core file name pattern: core.<hostname>.<pid>.<timestamp>
        '''
        coreinfo = []
        corepath = '/tmp/cores'
        gdbscript = \
        ...
        quit
        '''
        gdbpath = '/tmp/gdbscript'
        
        LOGGER.debug('Checking for core dumps in %s ...' % corepath)
        try:
            for filename in os.listdir(corepath):
                # We will append '.processed' to cores we have already seen
                if filename.endswith('processed'): 
                    ...
                    for file in os.listdir(corepath):
                        if os.stat(os.path.join(corepath, file)).st_mtime < one_week_ago:
                            print('remove %s' % os.path.join(corepath, file))
                            # os.unlink(somefile) # uncomment only if you are sure
                
                else:
                    corefile = os.path.join(corepath, filename)
                    LOGGER.debug('Found %s: processing' % corefile)
                    
                    # Write the gdb script to a file
                    with open(gdbpath, 'w') as f:
                        f.write(gdbscript)
                        
                    # Run the core through gdb to find executable name
                    output = Command('gdb -c %s -x %s' % (corefile, gdbpath), '.').execute()
                    regex = re.compile(".*Core was generated by `([\w/]+) .*", re.IGNORECASE | re.DOTALL)
                    r = regex.search(str(output[0]))
                    execname = r.groups(0)[0]

                    # Run through gdb again, with executable name
                    output = Command('gdb %s -c %s -x %s' % (execname, corefile, gdbpath), '.').execute()
                    coreinfo.append(str(output[0]))
                    
                    os.rename(corefile, corefile + '.processed')
